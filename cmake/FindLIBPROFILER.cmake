...
#   LIBPROFILER_FOUND - If false do not try to use libprofiler.
#   LIBPROFILER_INCLUDE_DIRS - where to find the headfile of library.
#   LIBPROFILER_LIBRARY_DIRS - where to find the libprofiler library.
#   LIBPROFILER_LIBRARIES, the library file name needed to use libprofiler.
#   LIBPROFILER_LIBRARY - the library needed to use libprofiler.
#
# Imported target
#   gperftools::profiler
...
if(NOT LIBPROFILER_FOUND)
  # windows系统下通过查找头文件 gperftools/profiler.h和find_library 查找profiler来实现 find the
  # headfile of library
  ...
  if(NOT LIBPROFILER_LIBRARY AND LIBPROFILER_INCLUDE_DIRS)
    ...
    find_library(
      ...
      PATHS ${LIBPROFILER_LIBRARY_DIRS})
  elseif(NOT LIBPROFILER_INCLUDE_DIRS AND LIBPROFILER_LIBRARY)
    ...
    get_filename_component(LIBPROFILER_LIBRARY_DIRS ${LIBPROFILER_LIBRARY}
                           DIRECTORY)
    find_path(LIBPROFILER_INCLUDE_DIRS ${PROFILER_HEADS}
              "${LIBPROFILER_LIBRARY_DIRS}../included")
  endif()
...
find_package_handle_standard_args(LIBPROFILER DEFAULT_MSG LIBPROFILER_LIBRARIES
                                  LIBPROFILER_INCLUDE_DIRS)

if(LIBPROFILER_FOUND)
  set(_static_libname libtcmalloc_and_profiler.a)
  find_library(
    LIBPROFILER_STATIC_LIBRARY
    NAMES ${_static_libname}
    PATHS ${LIBPROFILER_LIBRARY_DIRS})
  ...
  if(NOT TARGET gperftools::profiler)
    ...
    if(LIBPROFILER_STATIC_LINK)
      # for linking static profiler,must use libtcmalloc_and_profiler.a,see also
      # README gperftools
      set(_link_libs ${LIBPROFILER_STATIC_LDFLAGS})
      if(NOT LIBPROFILER_STATIC_LIBRARY)
        message(
          FATAL_ERROR
            "NOT FOUND static library for profiler:${_static_libname} ")
      endif()
    ...
    set_target_properties(
      gperftools::profiler
      PROPERTIES INTERFACE_COMPILE_OPTIONS "${LIBPROFILER_CFLAGS_OTHER}"
                 INTERFACE_INCLUDE_DIRECTORIES "${LIBPROFILER_INCLUDE_DIRS}"
                 INTERFACE_LINK_LIBRARIES "${_link_libs}")
    if(NOT LIBPROFILER_FIND_QUIETLY)
      message(
        STATUS
          "IMPORTED TARGET: gperftools::profiler,link libraies ${_link_libs}")
    endif()
  endif()
endif()
