...
bool dat_dict_add_index(match_dict_t dict, matcher_conf_t conf,
                        strlen_s keyword, strlen_s extra, void *tag,
                        mdi_prop_f prop) {
  matcher_conf_t stub_config = ((stub_conf_t) conf->buf)->stub;
  return stub_config->add_index(dict, stub_config, keyword, extra, tag, prop);
}

matcher_conf_t dat_matcher_conf(uint8_t id, matcher_type_e type,
                                matcher_conf_t stub) {
  matcher_conf_t conf =
      matcher_conf(id, type, dat_dict_add_index, sizeof(stub_conf_s));
  if (conf) {
    conf->clean = stub_config_clean;
    stub_conf_t stub_conf = (stub_conf_t) conf->buf;
    stub_conf->stub = stub;
  }
  return conf;
}
...
static void dat_construct_by_dfs(datrie_t self, trie_t origin,
  ...
  pDatNode->idxlist = pNode->idxlist;
  _retain(pDatNode->idxlist);
...
matcher_t dat_construct(match_dict_t dict, matcher_conf_t conf) {
  ...
  do {
    ...
    prime_trie = trie_construct(dict, &trie_conf);
    ...
    pdat->hdr._type = conf->type;
    pdat->hdr._func = dat_matcher_func;
  } while (0);
...
dat_context_t dat_alloc_context(datrie_t matcher) {
  ...
  ctx->hdr._type = matcher->hdr._type;
  ctx->hdr._func = acdat_context_func;

  if (ctx->hdr._type == matcher_type_seg_acdat) {
    ctx->hdr._func.next = (matcher_next_func) dat_seg_ac_next_on_index;
  } else if (ctx->hdr._type == matcher_type_prefix_acdat) {
    ctx->hdr._func.next = (matcher_next_func) dat_prefix_next_on_index;
  } else if (ctx->hdr._type == matcher_type_dat) {
    ctx->hdr._func.next = (matcher_next_func) dat_next_on_index;
  }
...
bool dat_match_end(dat_context_t ctx) {
  return ctx->_e >= ctx->hdr.content.len;
}

static inline size_t dat_forward(dat_node_t cur, dat_context_t ctx) {
  return cur->base + ((unsigned char *) ctx->hdr.content.ptr)[ctx->_e];
}

static inline void dat_output(dat_context_t ctx) {
  mdi_t mdi = _(list, ctx->_list, car);
  ctx->hdr.out_index = mdi;
