...
static matcher_t matcher_construct(vocab_t vocab, bool ignore_bad_pattern, bool bad_as_plain) {
  matcher_t matcher = matcher_alloc();

  // build reglet
  matcher->reglet = reglet_construct();
  if (!parse_vocab(vocab, add_pattern_to_matcher, matcher, ignore_bad_pattern, bad_as_plain)) {
...
matcher_t matcher_construct_by_file(const char* path, bool ignore_bad_pattern, bool bad_as_plain) {
  ...
  matcher_t matcher = matcher_construct(vocab, ignore_bad_pattern, bad_as_plain);
