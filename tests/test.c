...
int main()
{
  ...
  while (fscanf(fin, "%[^\n]\n", content) != EOF) {
    ...
    while (matcher_next(ctx1)) {
      // check history
      for (int i = (HISTORY_SIZE + hidx - hcount) % HISTORY_SIZE; i != hidx;
           i = (i + 1) % HISTORY_SIZE) {
        if (history[i].out_e <= ctx1->out_e) {
          hcount--;
        } else {
          long diff_pos =
              history[i].out_e - history[i].out_matched_index->length
                  - ctx1->out_e;
          if (diff_pos < 0) {
            continue;
          } else if (diff_pos > 45) {
...
check_w2:
        ...
        hcount++;
        long diff_pos =
            ctx2->out_e - ctx2->out_matched_index->length - ctx1->out_e;
        if (diff_pos < 0) {
          continue;
        } else if (diff_pos > 45) {
          goto next_round;
