...
class Matcher:
    ...
    def load_from_string(self, keyword, all_as_plain=False, ignore_bad_pattern=False, bad_as_plain=True, deduplicate_extra=True):
        if self._matcher:
            raise MatcherError("matcher is initialized")
        if keyword is None:
            return False
        self._matcher = _actrie.ConstructByString(
            convert2pass(keyword), all_as_plain, ignore_bad_pattern, bad_as_plain, deduplicate_extra)
        return self._matcher is not None

    @classmethod
    def create_by_string(cls, keyword, all_as_plain=False, ignore_bad_pattern=False, bad_as_plain=True, deduplicate_extra=True):
        matcher = cls()
        if matcher.load_from_string(keyword, all_as_plain, ignore_bad_pattern, bad_as_plain, deduplicate_extra):
            return matcher
        return None

    def load_from_collection(self, strings, all_as_plain=False, ignore_bad_pattern=False, bad_as_plain=True, deduplicate_extra=True):
        if self._matcher:
            raise MatcherError("matcher is initialized")
    ...
    @classmethod
    def create_by_collection(cls, strings, all_as_plain=False, ignore_bad_pattern=False, bad_as_plain=True, deduplicate_extra=True):
        matcher = cls()
        if matcher.load_from_collection(strings, all_as_plain, ignore_bad_pattern, bad_as_plain, deduplicate_extra):
            return matcher
        return None
